...
function createCard(recipe, index) {
  ...
  price.textContent = `Price: RM${recipe.price.toFixed(2)}`;
  price.style.color = '#ff4081';
  price.style.fontWeight = '700';
  price.style.margin = '4px 0 10px 0';
...
function addToCart(index) {
  ...
  alert(`Added ${recipes[index].title} to cart.`);
  updateCartDisplay();
}
...
function updateCartDisplay() {
  ...
  cart.forEach(item => {
    const recipe = recipes[item.index];
    const itemTotal = recipe.price * item.quantity;
    total += itemTotal;

    const li = document.createElement('li');
    li.textContent = `${recipe.title} x${item.quantity} - RM${itemTotal.toFixed(2)}`;
...
function checkout() {
  ...
  const total = cart.reduce((sum, item) => sum + recipes[item.index].price * item.quantity, 0);
  alert(`Thank you for ordering!\nYour total is RM${total.toFixed(2)}`);
  cart = [];
  updateCartDisplay();
  cartModal.style.display = 'none';
}
